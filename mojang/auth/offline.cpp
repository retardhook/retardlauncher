...
using namespace mojang::auth;

UserInfo Offline::Login(const std::string &username) {
  UserInfo user_info;
  user_info.username = username;
  user_info.uuid = GenerateUUID(username);

  // i dont think servers really check access and sesion token since theres no
  // way to check if it is real or wtv
  user_info.access_token = GenerateRandomString(32);
  user_info.session_token = GenerateRandomString(32);

  return user_info;
}
std::vector<unsigned char> Offline::ComputeMD5(const std::string &input) {
...
std::string Offline::GenerateUUID(const std::string &username) {
  ...
  uuid << "-";
  uuid << std::hex << std::setw(4) << std::setfill('0')
       << ((msb >> 16) & 0xFFFF);
  uuid << "-";
  ...
  uuid << std::hex << std::setw(4) << std::setfill('0')
       << ((lsb >> 48) & 0xFFFF);
  uuid << "-";
  uuid << std::hex << std::setw(12) << std::setfill('0')
       << (lsb & 0xFFFFFFFFFFFFULL);

  return uuid.str();
}

std::string Offline::GenerateRandomString(std::size_t length) {
